...
export async function createMerchantProduct(pool, merchantId, userId, body) {
  ...
  try { await client.query('BEGIN');
    ...
    for(const v of (body.variants||[])) { const vn=String(v?.name??'').trim(),sku=v?.sku==null?'':String(v.sku).trim(); const price=v?.price===null||v?.price===undefined||v?.price===''?null:Number(v.price),stock=Number(v?.stock??0); if(!vn||vn.length>200||sku.length>120||(price!==null&&(!Number.isFinite(price)||price<0))||!Number.isInteger(stock)||stock<0||(v?.active!==undefined&&typeof v.active!=='boolean')){const x=new Error('invalid_variants');x.status=400;throw x;} if(sku){const key=sku.toLowerCase();if(variantSkus.has(key)){const x=new Error('duplicate_variant_sku');x.status=400;throw x;}variantSkus.add(key);} await client.query(`INSERT INTO product_variants(product_id,name,sku,price,stock,active,source_variant_id) VALUES($1,$2,$3,$4,$5,$6,$7)`,[product.rows[0].id,vn,sku||null,price,stock,v?.active!==false,v?.source_variant_id||null]); }
