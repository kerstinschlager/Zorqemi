...
export async function createCheckoutSession(pool, body) {
  ...
  if (!items.length || items.length > 100) { const error = new Error('invalid_checkout_items'); error.status = 400; throw error; }
  ...
  const result = await pool.query(
    `SELECT p.id, p.merchant_id, p.name, p.description, p.price, p.stock, p.active
      ...
      WHERE p.id = ANY($1::uuid[]) AND p.active = true AND m.published = true`, [ids]
  );
  ...
  if (normalized.some((item) => !item)) { const error = new Error('product_unavailable'); error.status = 409; throw error; }
  ...
  const settingsResult = merchantIds.length === 1 ? await pool.query('SELECT * FROM shop_settings WHERE merchant_id = $1', [merchantIds[0]]) : { rows: [] };
  const settings = settingsResult.rows[0] || { currency: 'EUR', shipping_flat: 0, free_shipping_from: null, vat_enabled: false, vat_rate: 0 };
  const shipping = settings.free_shipping_from != null && subtotal >= Number(settings.free_shipping_from) ? 0 : Number(settings.shipping_flat || 0);
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: normalized.map(({ product, quantity }) => ({ quantity, price_data: {
        currency: String(settings.currency || 'EUR').toLowerCase(), unit_amount: money(product.price),
...
export async function handleStripeWebhook(pool, rawBody, signature) {
  ...
  if (!stripe || !secret) { const error = new Error('stripe_webhook_not_configured'); error.status = 503; throw error; }
  const event = stripe.webhooks.constructEvent(rawBody, signature || '', secret);
  const inserted = await pool.query(
    `INSERT INTO payment_events(provider,event_id,event_type,payload) VALUES ('stripe',$1,$2,$3)
     ON CONFLICT (provider,event_id) DO NOTHING RETURNING id`, [event.id, event.type, event]
  );
  ...
  if (event.type === 'checkout.session.completed' || event.type === 'checkout.session.async_payment_succeeded') {
    ...
    try {
      ...
      for (const item of items.rows) {
        const stock = await client.query(`UPDATE products SET stock=stock-$1,updated_at=now() WHERE id=$2 AND stock >= $1 RETURNING id`, [item.quantity, item.product_id]);
      ...
      const order = await client.query(
        `INSERT INTO orders(merchant_id,customer_id,status,currency,subtotal,shipping_total,tax_total,total,shipping_address,payment_provider,payment_reference)
         VALUES($1,NULL,'paid',$2,$3,$4,$5,$6,$7,'stripe',$8) RETURNING id`,
      ...
      for (const item of items.rows) {
        await client.query(
          `INSERT INTO order_items(order_id,product_id,merchant_id,product_name,quantity,unit_price,total) VALUES($1,$2,$3,$4,$5,$6,$7)`,
