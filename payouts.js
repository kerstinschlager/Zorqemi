    ...
    if(window.serverSession&&window.serverFetch&&window.serverSession.role==='admin'){
      try{
        ...
        admin$('#payoutTable').querySelectorAll('[data-payout-id]').forEach(b=>b.addEventListener('click',async()=>{
          try{const {response,payload}=await window.serverFetch('/admin/payouts/'+encodeURIComponent(b.dataset.payoutId)+'/paid',{method:'PATCH',headers:{'Content-Type':'application/json'},body:'{}'});if(!response.ok)throw new Error(payload?.error||'payout_update_failed');notifyAdmin('Auszahlung als bezahlt markiert');await renderPayoutControls();}catch(e){notifyAdmin(e.message||'Auszahlung konnte nicht aktualisiert werden')}
        }));
        return;
