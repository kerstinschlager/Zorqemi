  ...
  async function renderPayoutControls(){
    ...
    if(window.serverSession&&window.serverFetch&&window.serverSession.role==='admin'){
      try{
        ...
        admin$('#payoutTable').querySelectorAll('.payout-cancel-btn').forEach(b=>b.addEventListener('click',async()=>{try{await window.serverFetch('/admin/payouts/'+encodeURIComponent(b.dataset.payoutId)+'/cancel',{method:'PATCH',headers:{'Content-Type':'application/json'},body:'{}'});notifyAdmin('Auszahlung storniert');await renderPayoutControls();}catch(e){notifyAdmin(e.message||'Auszahlung konnte nicht storniert werden')}}));
