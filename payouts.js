(function(){
  async function renderPayoutControls(){
    ...
    if(window.serverSession&&window.serverFetch&&window.serverSession.role==='admin'){
      try{
        const {response,payload}=await window.serverFetch('/admin/payouts?limit=100');
        if(!response.ok)throw new Error(payload?.error||'payouts_load_failed');
        const rows=payload.payouts||[];
        admin$('#payoutTable').innerHTML=rows.map(p=>\`<div class="admin-table-row"><div><strong>\${adminEsc(p.merchant_name||'Händler')}</strong><span>\${new Date(p.created_at).toLocaleDateString('de-DE')} · Auszahlung #\${String(p.id).slice(0,8)}</span></div><div><strong>\${adminMoney(p.net_amount)}</strong><small>\${adminEsc(p.status)} · Provision \${adminMoney(p.commission_amount)}</small>\${p.status==='pending'?'<button type="button" class="secondary payout-paid-btn" data-payout-paid-id="'+p.id+'">Als bezahlt markieren</button><button type="button" class="secondary payout-cancel-btn" data-payout-id="'+p.id+'">Stornieren</button>':'<span class="muted">Bezahlt am '+(p.paid_at?new Date(p.paid_at).toLocaleDateString('de-DE'):'–')+'</span>'}</div></div>\`).join('')||'<p class="muted">Noch keine Auszahlungen.</p>';
        admin$('#payoutTable').querySelectorAll('.payout-cancel-btn').forEach(b=>b.addEventListener('click',async()=>{try{const {response,payload}=await window.serverFetch('/admin/payouts/'+encodeURIComponent(b.dataset.payoutId)+'/cancel',{method:'PATCH',headers:{'Content-Type':'application/json'},body:'{}'});if(!response.ok)throw new Error(payload?.error||'payout_cancel_failed');notifyAdmin('Auszahlung storniert');await renderPayoutControls();}catch(e){notifyAdmin(e.message||'Auszahlung konnte nicht storniert werden')}}));
        admin$('#payoutTable').querySelectorAll('[data-payout-paid-id]').forEach(b=>b.addEventListener('click',async()=>{
          try{const {response,payload}=await window.serverFetch('/admin/payouts/'+encodeURIComponent(b.dataset.payoutPaidId)+'/paid',{method:'PATCH',headers:{'Content-Type':'application/json'},body:'{}'});if(!response.ok)throw new Error(payload?.error||'payout_update_failed');notifyAdmin('Auszahlung als bezahlt markiert');await renderPayoutControls();}catch(e){notifyAdmin(e.message||'Auszahlung konnte nicht aktualisiert werden')}
        }));
        return;
